...
class StatsTest:
    ...
    def autostatstest(self, *columns):
        ...
        if num_cols == 2:
            ...
            if self.data[columns[0]].dtype in self.cat_dtypes and self.data[columns[1]].dtype in self.cat_dtypes:
                ...
                if observed.min() < 5 and fishers:
                    ...
                    result = fishers_test(observed_df, self.alpha)
                    return result
                # else conduct chi2 test
                result = chisquare_test(observed_df, self.alpha)
                results["chi2"] = result    
                ...
                if result.significant:
                    ...
                    if self.data[columns[1]].nunique() > 2:
                        try:
                            result = chisquare_posthoc_test(observed_df, self.alpha)
                            results["post-hoc"] = result
                        except ValueError:
                            warnings.warn("Post-hoc analysis cannot be conducted. Internally computed table of expected frequencies has zero elements.")
                else:
                    return result
            ...
            # continuous vs continuous 
                ...
                if normality[columns[0]] and normality[columns[1]]:
                    result = pearsonr_test(self.data[columns[0]], self.data[columns[1]], self.alpha)
                    return result
                # conduct spearman's correlation if one or both columns have non-normal distribution
                    ...
                    result = spearmanr_test(self.data[columns[0]], self.data[columns[1]], self.alpha)
                    return result
            ...
            # continuous vs categorical
                ...
                if self.data[columns[1]].nunique() == 2:
                    ...
                    if normality[columns[0]]:
                        ...
                        result = t_test(sample1, sample2, alpha = self.alpha, equal_variance = equal_var)
                        return result
                    else:
                        ...
                        result = mannwhitneyu_test(sample1, sample2, alpha = self.alpha)
                        return result
                    ...
                    if normality[columns[0]]: 
                        ...
                        result = onewayanova_test(*samples, alpha = self.alpha, equal_variance = equal_var)
                        results["anova"] = result
                        # conduct post-hoc analysis using tukey's HSD test if one-way ANOVA test is significant
                        if result.significant:
                            df = pd.DataFrame({columns[0]: self.data[columns[0]], columns[1]: self.data[columns[1]]})
                            result = tukeys_test(df, columns[0], columns[1], alpha = self.alpha)
                            results["post-hoc"] = result
                        else:
                            return result
                    ...
                    # conduct kruskal-wallis test if dependent variable is not normally distributed
                        ...
                        result = kruskalwallis_test(*samples, alpha = self.alpha, equal_variance = equal_var)
                        results["kw"] = result
                        # conduct post-hoc analysis using dunn's test if kruskal-wallis test is significant
                        if result.significant:
                            df = pd.DataFrame({columns[0]: self.data[columns[0]], columns[1]: self.data[columns[1]]})
                            result = dunns_test(df, columns[0], columns[1], alpha = self.alpha)
                            results["post-hoc"] = result
                        else:
                            return result
    ...
    def autostatstest_all(self, dependent_col):
            ...
            if column != dependent_col:
                try:
                    ...
                    with warnings.catch_warnings(record = True) as w:
                        result = self.autostatstest(dependent_col, column)
                        if w:
